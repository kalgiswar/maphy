...
const CategoriesConsumables = () => {
  ...
  const { rowData } = locationpath.state || {}; // Add a default value
  console.log("row datatata:", rowData);

  const getDetails = async () => {
    const url = `${Domain}/consumables?category_id=${id}&limit=${
          ...
          <Col md={12}>
            <div>
              <div className="title d-flex justify-content-between">
                <div>
                  <h1>
                    {t("category.Consumable")} - {rowData?.name}
                  </h1>
                </div>
                ...
                <div className="d-flex justify-content-between">
                  <div className="my-3">
                    <Form>
                      ...
                      <CsvExportButton
                        headers={[
                          t("CategoryConsumable.id"),
                          t("CategoryConsumable.name"),
                          t("CategoryConsumable.category"),
                          t("CategoryConsumable.total"),
                          t("CategoryConsumable.remaining"),
                          t("CategoryConsumable.minqty"),
                          t("CategoryConsumable.order_number"),
                        ]}
                        data={csvData}
                        filename="Audit.csv"
                      />
                ...
                <Table>
                  <thead>
                    <tr>
                      ...
                      <th>{t("CategoryConsumable.category")} </th>
                      <th>
                        {t("CategoryConsumable.total")}
                        <SortingIcon
                          columnName="qty"
                      ...
                      <th>{t("CategoryConsumable.order_number")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data?.length > 0 ? (
...
export default CategoriesConsumables;
