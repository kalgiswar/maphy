...
const MainPage = () => {
  ...
  const getDetails = async () => {
    setLoading(true);
    const url = `${Domain}/companies?limit=${sizePerPage}&offset=${offset}&search=${searchText}&sort=${sortField}&order=${sortOrder}`;
    try {
  ...
  const deleteItem = async () => {
    const url = `${Domain}/companies/${itemTodelete}`;
    try {
          ...
          <Col md={12}>
            <div>
              <div className="title d-flex justify-content-between">
                <div>
                  <h1>{t("company.companytitle")}</h1>
                </div>
                <div>
                  <Button
                    className="primary px-4"
                    type="submit"
                    onClick={() => navigate("/addEditCompany")}
                  >
                    {t("button.create")}
                  </Button>
                </div>
                    ...
                    <Form>
                      ...
                      <CsvExportButton
                        headers={[
                          ...
                          t("company.assets"),
                          t("company.licenses"),
                          t("company.accessories"),
                          t("company.consumables"),
                          t("company.components"),
                        ]}
                        data={csvData}
                  ...
                  <Table>
                    <thead>
                      <tr>
                        ...
                        <th>{t("company.people")}</th>
                        <th>{t("company.assets")}</th>
                        <th>{t("company.licenses")}</th>
                        <th>{t("company.accessories")}</th>
                        <th>{t("company.consumables")}</th>
                        <th>{t("company.components")}</th>
                        <th className="right-radius">{t("company.actions")}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {loading ? (
                          ...
                          <tr key={index}>
                            <td>
                              <button
                                ...
                                onClick={() =>
                                  navigate(`/companiesDetails/${items?.id}`, {
                                    state: {
                                      rowData: items,
                                      activeTab: "people",
                                    },
