...
const MainPage = () => {
  ...
  const getDetails = async () => {
    const url = `${Domain}/hardware?supplier_id=${id}&limit=${sizePerPage}&offset=${offset}&search=${searchText}&sort=${sortField}&order=${sortOrder}`;
    try {
                    ...
                    {data?.length > 0 ? (
                      data.map((items, index) => (
                        <tr key={index}>
                          ...
                          <td>{items?.[0]?.assigned_to?.name}</td>
