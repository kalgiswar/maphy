...
router.get('/', errorHandler(async function(req, res, next) {
    ...
    let result = await department.findAndCountAll({
        attributes: ['id', 'name', 'image', 'created_at', 'updated_at', [Sequelize.literal('(SELECT COUNT(`users`.`department_id`) FROM `users` WHERE `users`.`department_id` = `department`.`id`)'), 'users_count'],],
        include: [
