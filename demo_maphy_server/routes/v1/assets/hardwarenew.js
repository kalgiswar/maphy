...
const depreciation = require('../../../db/models/depreciation');
...
router.get('/', errorHandler(async function (req, res, next) {
   ...
   // model.belongsTo(manufacturer, { foreignKey: 'manufacturer_id' });
    ...
    const check_out_type = await util.getItemType1(queries.checkout_to_type);
    ...
    if (!_.isNil(requestable) && _.eq(requestable, '1')) {
       ...
       // util.addCondition(queries.company_id, where, { company_id: queries.company_id })
        ...
        util.addCondition(queries.checkout_to_type, where, { assigned_type:check_out_type })
        ...
        let result = await asset.findAndCountAll({
            attributes: ['id', 'assigned_to', 'asset_tag', 'requestable', 'order_number', 'assetdetails', 'checkin_counter', 'checkout_counter', 'expected_checkin', 'image', 'last_audit_date', 'last_checkout', 'name', 'next_audit_date', 'notes', 'order_number', 'purchase_cost', 'tax_value', 'excluding_tax', 'purchase_date', 'serial', 'warranty_months', 'requestable', 'created_at', 'updated_at', 'deleted_at','pvnumber','pv_date','capitlaization_date','warranty_from_date','warranty_to_date','depreciation_id',
                'assigned_type', [
                    Sequelize.literal(
                        '(SELECT CASE WHEN assigned_type = "App\\\\Models\\\\Location" THEN (SELECT name FROM locations WHERE id = `asset`.`assigned_to`) WHEN assigned_type = "App\\\\Models\\\\User" THEN (SELECT CONCAT(username, " ", employee_num) as name FROM users WHERE id = `asset`.`assigned_to`) WHEN assigned_type = "App\\\\Models\\\\Asset" THEN (SELECT name FROM assets WHERE id = `asset`.`assigned_to`)  WHEN assigned_type = "App\\\\Models\\\\Department" THEN (SELECT name FROM departments WHERE id = `asset`.`assigned_to`)  END)'), 'assigned'],
...
async function getAsset(where) {
   ...
   // model.belongsTo(manufacturer, { foreignKey: 'manufacturer_id' });
    ...
    let result = await asset.findOne({
        attributes: ['id', 'asset_tag', 'requestable', 'order_number', 'assetdetails', 'checkin_counter', 'checkout_counter', 'expected_checkin', 'image', 'last_audit_date', 'last_checkout', 'name', 'next_audit_date', 'notes', 'order_number', 'purchase_cost', 'purchase_date', 'serial', 'warranty_months', 'requestable', 'created_at', 'updated_at', 'deleted_at','pvnumber','pv_date','capitlaization_date','warranty_from_date','warranty_to_date','depreciation_id'],
        include: [
            { model: statusLabel, required: true, attributes: ['id', 'name'] },
            //{ model: location, attributes: ['id', 'name'] },
            { model: depreciation, attributes: ['id', 'name'] },
    ...
    await sequelize.transaction(async t => {
        for (let a of request.assets) {
            ...
            let assetRequest = {
                ...
                warranty_to_date:req.body.warranty_to_date,
                warranty_from_date:req.body.warranty_from_date,
                pvnumber:req.body.pvnumber,
                pv_date:req.body.pv_date,
                capitlaization_date:req.body.capitlaization_date,
...
router.post('/:id/checkout', errorHandler(async function (req, res, next) {
    const actionLogs = models.actionLogs;
    const type = await util.getItemType(req.body)
    console.log("ccccccccccccccccccccccc:",type);
...
router.put('/:id', errorHandler(async function (req, res, next) {
    ...
    await sequelize.transaction(async t => {
        ...
        res.result = { error: "Something went wrong" }
        console.log(err)
    });
...
function getAssigned(asset) {
    ...
    switch (asset.assigned_type) {
        ...
        case constants.itemTypes.asset:
            assigned.type = 'asset'
            break;
        case constants.itemTypes.department:
...
function formatResponse(asset) {
    ...
    return {
       ...
       // eol: asset.model.eol,
        ...
        warranty_from_date:util.createdUpdatedDateFormat(asset.warranty_from_date),
        warranty_to_date:util.createdUpdatedDateFormat(asset.warranty_from_date),
        pvnumber:asset.pvnumber,
        pv_date:util.createdUpdatedDateFormat(asset.pv_date),
        capitlaization_date:util.createdUpdatedDateFormat(asset.capitlaization_date),
...
function formatAssetResponse(asset) {
    ...
    return {
       ...
       manufacturer: util.getRelationalObject(asset.model.manufacturer),
        ...
        warranty_expires: util.createdUpdatedDateFormat(asset.warranty_expires),
        warranty_months: asset.warranty_months,
        warranty_from_date:asset.warranty_from_date,
        warranty_to_date:asset.warranty_from_date,
        pvnumber:asset.pvnumber,
        pv_date:asset.pv_date,
        capitlaization_date:asset.capitlaization_date,
        depreciation: util.getRelationalObject(asset.depreciation),
    }
}
